...
public class Main {
    ...
    public static void main(String[] args) {
        if (args.length < 2) {
            System.err.println("Usage: ldoc <repository> <path-to-source-root> [module] [--with-summary] [--with-embeddings]");
            System.err.println();
        ...
        log.info("Starting ldoc: repo={}, module={}, source={}, withSummary={}, withEmbeddings={}",
                repository, module, sourceDir.toAbsolutePath(), withSummary, withEmbeddings);

        AppConfig config = new AppConfig();
        AnalysisPipeline pipeline = new AnalysisPipeline(config);
        pipeline.run(sourceDir, repository, module, withSummary, withEmbeddings);
